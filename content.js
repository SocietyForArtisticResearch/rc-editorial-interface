...
// Function to update disable button appearance based on current state
async function updateDisableButtonState() {
    try {
        const currentState = await browser.storage.local.get('extensionDisabled');
        const isDisabled = currentState.extensionDisabled || false;
        
        const disableButton = document.getElementById('rc-disable-extension-btn');
        const buttonText = document.getElementById('rc-disable-button-text');
        
        if (disableButton && buttonText) {
            if (isDisabled) {
                disableButton.style.background = 'linear-gradient(135deg, #28a745, #20c997)';  // Green when disabled (to enable)
                buttonText.textContent = 'Exit';
                disableButton.title = 'Click to enable extension';
            } else {
                disableButton.style.background = 'linear-gradient(135deg, #dc3545, #c82333)';  // Red when enabled (to disable)
                buttonText.textContent = 'Exit';
                disableButton.title = 'Click to disable extension';
            }
        }
    } catch (error) {
        console.error('Error updating disable button state:', error);
    }
}
...
async function enhanceTools(toolsToEnhance = null) {
    ...
    // Ensure all existing spans have click handlers
    const allTextTools = document.querySelectorAll('.tool-text, .tool-simpletext');
